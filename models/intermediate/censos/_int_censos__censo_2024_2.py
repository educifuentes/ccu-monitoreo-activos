...
def int_censos__censo_2024_2_deprecated():
    ...
    # rename columns
    int_censos__censo_2024_2_df = stg_censos_1_df.rename(columns=rename_dict)

    # new columns
    int_censos__censo_2024_2_df["periodo"] = "2024-S2"
    ...
    # Add period
    int_censos__censo_2024_2_df["periodo"] = "2024-S2" # Assuming this is the previous period

    # Calculate total outputs (salidas) by summing machines 1 to 12
    int_censos__censo_2024_2_df["salidas"] = 0
    for i in range(1, 13):
        col_name = f"NÚMERO DE SALIDAS TOTALES DE SCHOPERAS\nMÁQUINA SCHOPERA {i}:"
        col_name_alt = f"NÚMERO DE SALIDAS TOTALES DE SCHOPERAS \nMÁQUINA SCHOPERA {i}:"
        
        if col_name in int_censos__censo_2024_2_df.columns:
            int_censos__censo_2024_2_df["salidas"] += pd.to_numeric(int_censos__censo_2024_2_df[col_name], errors='coerce').fillna(0)
        elif col_name_alt in int_censos__censo_2024_2_df.columns:
    ...
    available_columns = [col for col in selected_columns if col in int_censos__censo_2024_2_df.columns]
    
    return int_censos__censo_2024_2_df[available_columns]
