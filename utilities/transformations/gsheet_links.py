...
def add_gsheet_link(df, gid):
    """
    Adds a column with a direct link to the corresponding row in Google Sheets.
    Uses 'row_index' column if present to point to the specific row.
    """
    if not gid:
        return df
    ...
    if "row_index" in df.columns:
        ...
        df["ir a gsheet"] = df["row_index"].apply(lambda x: f"{base_url}&range=A{x}" if pd.notna(x) else base_url)
    else:
